...
destination "output_default_test-elasticsearch-out" {
	elasticsearch-http(index("xxx-${YEAR}.${MONTH}.${DAY}") url("test.local") headers("a:b" "c:d") batch-lines(2000) workers(3) persist_name("output_default_test-elasticsearch-out"));
};
`,
	)
}
